...
def format_report_to_console(
    report: ScanReport,
    mode: ConsoleMode = ConsoleMode.SUMMARY
) -> ConsoleOutput:
    """
...
def format_report_to_csv(report: ScanReport) -> str:
    ...
    # 헤더
    writer.writerow([
        "ID", "Plugin", "Severity", "Title", "URL", "Parameter",
        "Method", "Payload", "Evidence", "CWE ID", "CVSS Score",
...
<body>
    ...
    html_parts.append(f"<div class='summary-item'><strong>Target URL:</strong> {report.target_url}</div>")
    html_parts.append(f"<div class='summary-item'><strong>Scan ID:</strong> {report.scan_id}</div>")
    html_parts.append(f"<div class='summary-item'><strong>Scanner Version:</strong> {report.scanner_version}</div>")
    html_parts.append(f"<div class='summary-item'><strong>Start Time:</strong> {report.start_time.isoformat()}</div>")
    html_parts.append(f"<div class='summary-item'><strong>End Time:</strong> {report.end_time.isoformat()}</div>")
    html_parts.append(f"<div class='summary-item'><strong>Duration:</strong> {report.duration_seconds:.2f} seconds</div>")
    ...
    if report.summary:
        ...
        html_parts.append("<div class='summary'>")
        html_parts.append(f"<div class='summary-item'><strong>Total Vulnerabilities:</strong> {summary.total_vulnerabilities}</div>")
        html_parts.append(f"<div class='summary-item'><strong>Total URLs Scanned:</strong> {summary.total_urls_scanned}</div>")
        html_parts.append(f"<div class='summary-item'><strong>Total Requests:</strong> {summary.total_requests}</div>")
        html_parts.append(f"<div class='summary-item'><strong>Success Rate:</strong> {summary.success_rate:.1f}%</div>")
        
        if summary.severity_counts:
            html_parts.append("<div class='summary-item'><strong>Severity Breakdown:</strong></div>")
            html_parts.append("<ul>")
            for severity in [Severity.CRITICAL, Severity.HIGH, Severity.MEDIUM, Severity.LOW, Severity.INFO]:
                count = summary.severity_counts.get(severity, 0)
                if count > 0:
                    severity_class = f"severity-{severity.value.lower()}"
                    html_parts.append(f"<li class='{severity_class}'>{severity.value}: {count}</li>")
            html_parts.append("</ul>")
        html_parts.append("</div>")
    ...
    for plugin_result in report.plugin_results:
        if plugin_result.findings:
            ...
            html_parts.append(f"<p><strong>Status:</strong> {plugin_result.status.value}</p>")
            html_parts.append(f"<p><strong>Findings Count:</strong> {len(plugin_result.findings)}</p>")
            
            for finding in plugin_result.findings:
                severity_class = f"severity-{finding.severity.value.lower()}"
                html_parts.append(f"<div class='finding'>")
                html_parts.append(f"<h4 class='{severity_class}'>[{finding.severity.value}] {finding.title}</h4>")
                html_parts.append(f"<p><strong>ID:</strong> {finding.id}</p>")
                if finding.url:
                    html_parts.append(f"<p><strong>URL:</strong> {finding.url}</p>")
                if finding.parameter:
                    html_parts.append(f"<p><strong>Parameter:</strong> {finding.parameter}</p>")
                if finding.method:
                    html_parts.append(f"<p><strong>Method:</strong> {finding.method}</p>")
                if finding.payload:
                    html_parts.append(f"<p><strong>Payload:</strong> <code>{finding.payload}</code></p>")
                if finding.description:
                    html_parts.append(f"<p><strong>Description:</strong> {finding.description}</p>")
                if finding.evidence:
                    html_parts.append(f"<p><strong>Evidence:</strong> {finding.evidence}</p>")
                if finding.remediation:
                    html_parts.append(f"<p><strong>Remediation:</strong> {finding.remediation}</p>")
                if finding.cwe_id:
                    html_parts.append(f"<p><strong>CWE ID:</strong> {finding.cwe_id}</p>")
                if finding.cvss_score:
                    html_parts.append(f"<p><strong>CVSS Score:</strong> {finding.cvss_score}</p>")
                html_parts.append(f"<p class='metadata'><strong>Confidence:</strong> {finding.confidence.value} | <strong>Timestamp:</strong> {finding.timestamp.isoformat()}</p>")
                html_parts.append("</div>")
